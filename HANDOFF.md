...
- ✅ **`install.html` defaults to the Web Store (production) ID** `fhgjojnifekiifaenchgammalffepbeg` (release v2.5.1). For local unpacked testing, override with `install.html?id=<unpacked-id>` — do NOT change the production default.
  ✅ **目前 `install.html` 預設是「正式版(Web Store)」ID** `fhgjojnifekiifaenchgammalffepbeg`(release v2.5.1)。本機未封裝測試請用 `install.html?id=<未封裝ID>` 覆寫,別動這個正式預設值。
...
- [x] In `docs/install.html`, set `DEFAULT_EXTENSION_ID = 'fhgjojnifekiifaenchgammalffepbeg'` (the Web Store ID). / 已把 `DEFAULT_EXTENSION_ID` 切成上架版 ID。**(done for v2.5.1 release)**
